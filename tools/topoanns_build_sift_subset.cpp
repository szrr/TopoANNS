...
struct Args {
    std::filesystem::path base_bvecs;
...
[[noreturn]] void Usage() {
    std::cerr
        << "Usage: topoanns_build_sift_subset"
        << " --base-bvecs <path>"
        << " --disk-index <path>"
        << " --pq-pivots <path>"
        << " --pq-compressed <path>"
...
Args ParseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (flag == "--base-bvecs") {
            args.base_bvecs = read_value("--base-bvecs");
    ...
    if (args.base_bvecs.empty() || args.disk_index.empty() || args.pq_pivots.empty() ||
...
void ConvertVectors(const Args& args,
                    const topoanns::BvecsMetadata& bvecs,
                    const std::filesystem::path& vector_store_path) {
    const topoanns::VectorPageLayout layout =
        topoanns::VectorPageLayout::Create(bvecs.dim, topoanns::ScalarKind::kFloat32);
    const topoanns::VectorStoreHeader header =
        BuildVectorStoreHeader(layout, bvecs.dim, args.num_vectors);

    std::ifstream in(args.base_bvecs, std::ios::binary);
    if (!in.is_open()) {
        throw std::runtime_error("Failed to open base bvecs: " + args.base_bvecs.string());
    }
    ...
    const std::size_t record_bytes = bvecs.record_bytes;
    std::vector<std::uint8_t> raw_records(args.chunk_vectors * record_bytes, 0);
    std::vector<float> float_chunk(args.chunk_vectors * bvecs.dim, 0.0f);
    ...
    while (processed < args.num_vectors) {
        ...
        const std::size_t bytes_to_read = static_cast<std::size_t>(current) * record_bytes;
        ...
        for (std::uint64_t i = 0; i < current; ++i) {
            const std::uint8_t* record = raw_records.data() + i * record_bytes;
            std::uint32_t dim = 0;
            std::memcpy(&dim, record, sizeof(dim));
            if (dim != bvecs.dim) {
                throw std::runtime_error("Per-record dimension mismatch in bvecs.");
            }
            const std::uint8_t* payload = record + sizeof(std::uint32_t);
            float* dst = float_chunk.data() + static_cast<std::size_t>(i) * bvecs.dim;
            for (std::uint32_t d = 0; d < bvecs.dim; ++d) {
                dst[d] = static_cast<float>(payload[d]);
            }
        }

        const std::uint8_t* float_bytes =
            reinterpret_cast<const std::uint8_t*>(float_chunk.data());
        const std::size_t vector_bytes = layout.vector_bytes();
        for (std::uint64_t i = 0; i < current; ++i) {
            const std::uint8_t* vector_src = float_bytes + i * vector_bytes;
        ...
        if (processed == args.num_vectors ||
            ...
            std::cout << "[topoanns_build] converted vectors " << processed << " / "
...
void ExtractTopology(const Args& args,
    ...
    if (layout.metadata().nodes_per_sector > 0) {
        ...
        for (std::uint64_t sector_id = 0; sector_id < total_sectors; ++sector_id) {
            ...
            if (processed == args.num_vectors ||
                ...
                std::cout << "[topoanns_build] extracted topology " << processed << " / "
                          << args.num_vectors << std::endl;
            }
        }
    } else {
    ...
    std::cout << "[topoanns_build] filtered out-of-range neighbors: "
...
void WriteManifest(const Args& args,
    ...
    out << "{\n"
        ...
        << "  \"source_base_bvecs\": \"" << args.base_bvecs.string() << "\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        const topoanns::BvecsMetadata bvecs = topoanns::ReadBvecsMetadata(args.base_bvecs);
        const topoanns::DiskannDiskIndexLayout disk_layout =
            topoanns::DiskannDiskIndexLayout::Load(args.disk_index);
        if (bvecs.dim != disk_layout.metadata().vector_dim) {
            throw std::runtime_error("bvecs dim and DiskANN vector dim do not match.");
        }
        if (args.num_vectors > bvecs.num_vectors ||
        ...
        ConvertVectors(args, bvecs, vector_store_path);
        ...
        WriteManifest(args, bvecs, disk_layout, vector_layout);

        std::cout << "[topoanns_build] done" << std::endl;
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "[topoanns_build] " << e.what() << std::endl;
        return 1;
    }
}
