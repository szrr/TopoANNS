...
[[noreturn]] void Usage() {
    std::cerr
        << "Usage: topoanns_bam_write_topology"
        << " --topology <path>"
...
Args ParseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (flag == "--topology") {
            args.topology_path = read_value("--topology");
...
int main(int argc, char** argv) {
    try {
        ...
        const topoanns::TopologyHeader header =
            topoanns::TopologyLayout::ReadHeader(args.topology_path);
        const std::uint64_t payload_bytes = header.payload_bytes;
        const std::uint64_t payload_bytes_padded =
            AlignUp(payload_bytes, topoanns::kDefaultPageSizeBytes);

        std::cout << "[topoanns_bam_write_topology]"
                  ...
                  << " payload_end_offset_bytes="
                  << (args.device_offset_bytes + payload_bytes_padded)
        ...
        RequireL40Device(bam_runtime.cuda_device, "topoanns_bam_write_topology");
        ...
        options.payload_bytes_override = static_cast<std::size_t>(payload_bytes_padded);
        topoanns::WriteFilePayloadToBam(args.topology_path,
                                        sizeof(topoanns::TopologyHeader),
                                        topoanns::kDefaultPageSizeBytes,
                                        options);
        std::cout << "[topoanns_bam_write_topology] completed" << std::endl;
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "[topoanns_bam_write_topology] " << e.what() << std::endl;
        return 1;
    }
}
