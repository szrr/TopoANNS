...
int main() {
    ...
    const auto pivots_path =
        std::filesystem::current_path() / "test_topoanns_search_fused_pivots.bin";
    const auto codes_path =
        std::filesystem::current_path() / "test_topoanns_search_fused_codes.bin";
    const auto vector_store_path =
        std::filesystem::current_path() / "test_topoanns_search_fused_vectors.ssd";
    ...
    params.rerank.top_n = 4;

    const auto baseline = topoanns::TopoAnnsSearch::RunBatchFloat32(
        resources, device_entries, queries, 2, params);
    for (const auto rerank_mode : {topoanns::RerankExecutionMode::kLinear,
                                   topoanns::RerankExecutionMode::kTiled,
                                   topoanns::RerankExecutionMode::kPageByPage}) {
        topoanns::TopoAnnsSearchParams mode_params = params;
        mode_params.rerank.mode = rerank_mode;
        mode_params.rerank.rank_tile_size = 2;
        const auto fused = topoanns::TopoAnnsSearch::RunBatchFloat32Fused(
            resources, device_entries, queries, 2, mode_params);

        TOPOANNS_ASSERT_EQ(fused.queries.size(), baseline.queries.size());
        for (std::size_t query_id = 0; query_id < baseline.queries.size(); ++query_id) {
            TOPOANNS_ASSERT_EQ(fused.queries[query_id].topology.stats.visited_nodes,
                               baseline.queries[query_id].topology.stats.visited_nodes);
            TOPOANNS_ASSERT_EQ(fused.queries[query_id].topology.stats.expanded_nodes,
                               baseline.queries[query_id].topology.stats.expanded_nodes);
            TOPOANNS_ASSERT_EQ(fused.queries[query_id].rerank.topk.size(),
                               baseline.queries[query_id].rerank.topk.size());
            for (std::size_t rank = 0; rank < baseline.queries[query_id].rerank.topk.size();
                 ++rank) {
                TOPOANNS_ASSERT_EQ(fused.queries[query_id].rerank.topk[rank].node_id,
                                   baseline.queries[query_id].rerank.topk[rank].node_id);
                TOPOANNS_ASSERT_EQ(fused.queries[query_id].rerank.topk[rank].distance,
                                   baseline.queries[query_id].rerank.topk[rank].distance);
            }
        }
    }

    std::cout << "test_topoanns_search_fused passed" << std::endl;
    return 0;
}
