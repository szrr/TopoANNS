...
int main() {
    ...
    const auto pivots_path =
        std::filesystem::current_path() / "test_topoanns_search_device_entries_pivots.bin";
    const auto codes_path =
        std::filesystem::current_path() / "test_topoanns_search_device_entries_codes.bin";
    const auto vector_store_path =
        std::filesystem::current_path() / "test_topoanns_search_device_entries_vectors.ssd";
    ...
    topoanns::FixedEntryProvider host_entries({0U});
    ...
    const auto host_result = topoanns::TopoAnnsSearch::RunBatchFloat32(
        resources, host_entries, queries, 2, params);
    const auto device_result = topoanns::TopoAnnsSearch::RunBatchFloat32(
        resources, device_entries, queries, 2, params);

    TOPOANNS_ASSERT_EQ(device_result.queries.size(), host_result.queries.size());
    TOPOANNS_ASSERT_EQ(device_result.rerank_stats.exact_distance_count,
                       host_result.rerank_stats.exact_distance_count);
    TOPOANNS_ASSERT_EQ(device_result.rerank_stats.io_pages, host_result.rerank_stats.io_pages);
    for (std::size_t query_id = 0; query_id < host_result.queries.size(); ++query_id) {
        TOPOANNS_ASSERT_EQ(device_result.queries[query_id].topology.topk.size(),
                           host_result.queries[query_id].topology.topk.size());
        TOPOANNS_ASSERT_EQ(device_result.queries[query_id].rerank.topk.size(),
                           host_result.queries[query_id].rerank.topk.size());
        for (std::size_t rank = 0; rank < host_result.queries[query_id].topology.topk.size();
             ++rank) {
            TOPOANNS_ASSERT_EQ(device_result.queries[query_id].topology.topk[rank].node_id,
                               host_result.queries[query_id].topology.topk[rank].node_id);
        }
        for (std::size_t rank = 0; rank < host_result.queries[query_id].rerank.topk.size();
             ++rank) {
            TOPOANNS_ASSERT_EQ(device_result.queries[query_id].rerank.topk[rank].node_id,
                               host_result.queries[query_id].rerank.topk[rank].node_id);
            TOPOANNS_ASSERT_EQ(device_result.queries[query_id].rerank.topk[rank].distance,
                               host_result.queries[query_id].rerank.topk[rank].distance);
        }
    }

    std::cout << "test_topoanns_search_device_entries passed" << std::endl;
    return 0;
}
