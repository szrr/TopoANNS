...
#define IO_DEPTH_HISTOGRAM_BINS 4097
#define FREE 2
...
} __attribute__((aligned (16))) data_page_t;

static_assert(sizeof(data_page_t) == 16,
              "BaM per-page state must remain compact for billion-scale ranges");
...
struct page_cache_d_t {
    ...
    simt::atomic<uint64_t, simt::thread_scope_device>* total_gpu_hit_cnt;
    simt::atomic<uint64_t, simt::thread_scope_device>* total_host_hit_cnt;
    simt::atomic<uint64_t, simt::thread_scope_device>* total_ssd_miss_cnt;
    simt::atomic<uint32_t, simt::thread_scope_device>* io_depth_profile_enabled;
    uint32_t io_depth_histogram_bins;
    simt::atomic<uint64_t, simt::thread_scope_device>* io_submitted_cnt;
    simt::atomic<uint64_t, simt::thread_scope_device>* io_completed_cnt;
    simt::atomic<uint64_t, simt::thread_scope_device>* io_current_outstanding;
    simt::atomic<uint64_t, simt::thread_scope_device>* io_depth_histogram;
...
struct page_cache_t {
    ...
    BufferPtr total_gpu_hit_cnt_buf;
    BufferPtr total_host_hit_cnt_buf;
    BufferPtr total_ssd_miss_cnt_buf;
    BufferPtr io_depth_profile_enabled_buf;
    BufferPtr io_submitted_cnt_buf;
    BufferPtr io_completed_cnt_buf;
    BufferPtr io_current_outstanding_buf;
    BufferPtr io_depth_histogram_buf;
...
//        printf("Cache Extra Reads: %llu\n", v);
    ...
    page_cache_t(const uint64_t ps, const uint64_t np, const uint32_t cudaDevice, const Controller& ctrl, const uint64_t max_range, const std::vector<Controller*>& ctrls) {
        ...
        total_gpu_hit_cnt_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        total_host_hit_cnt_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        total_ssd_miss_cnt_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        io_depth_profile_enabled_buf = createBuffer(sizeof(simt::atomic<uint32_t, simt::thread_scope_device>), cudaDevice);
        io_submitted_cnt_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        io_completed_cnt_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        io_current_outstanding_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        io_depth_histogram_buf = createBuffer(IO_DEPTH_HISTOGRAM_BINS * sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        ...
        pdt.total_gpu_hit_cnt = (simt::atomic<uint64_t, simt::thread_scope_device>*)total_gpu_hit_cnt_buf.get();
        pdt.total_host_hit_cnt = (simt::atomic<uint64_t, simt::thread_scope_device>*)total_host_hit_cnt_buf.get();
        pdt.total_ssd_miss_cnt = (simt::atomic<uint64_t, simt::thread_scope_device>*)total_ssd_miss_cnt_buf.get();
        pdt.io_depth_profile_enabled = (simt::atomic<uint32_t, simt::thread_scope_device>*)io_depth_profile_enabled_buf.get();
        pdt.io_depth_histogram_bins = IO_DEPTH_HISTOGRAM_BINS;
        pdt.io_submitted_cnt = (simt::atomic<uint64_t, simt::thread_scope_device>*)io_submitted_cnt_buf.get();
        pdt.io_completed_cnt = (simt::atomic<uint64_t, simt::thread_scope_device>*)io_completed_cnt_buf.get();
        pdt.io_current_outstanding = (simt::atomic<uint64_t, simt::thread_scope_device>*)io_current_outstanding_buf.get();
        pdt.io_depth_histogram = (simt::atomic<uint64_t, simt::thread_scope_device>*)io_depth_histogram_buf.get();
        cuda_err_chk(cudaMemset(pdt.total_gpu_hit_cnt, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.total_host_hit_cnt, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.total_ssd_miss_cnt, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.io_depth_profile_enabled, 0, sizeof(uint32_t)));
        cuda_err_chk(cudaMemset(pdt.io_submitted_cnt, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.io_completed_cnt, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.io_current_outstanding, 0, sizeof(uint64_t)));
        cuda_err_chk(cudaMemset(pdt.io_depth_histogram, 0,
                                pdt.io_depth_histogram_bins * sizeof(uint64_t)));
    ...
    do {
        ...
        case NV_NB:
            ...
            if ((st_new & BUSY) == 0) {
                ...
                cache.total_ssd_miss_cnt->fetch_add(1, simt::memory_order_relaxed);
        ...
        case V_NB:
            ...
            hit_cnt.fetch_add(count, simt::memory_order_relaxed);
            cache.total_gpu_hit_cnt->fetch_add(1, simt::memory_order_relaxed);
            return page_trans;
...
inline __device__ void read_data(page_cache_d_t* pc, QueuePair* qp, const uint64_t starting_lba, const uint64_t n_blocks, const unsigned long long pc_entry) {
    ...
    if (pc->io_depth_profile_enabled->load(simt::memory_order_relaxed) != 0) {
        const uint64_t depth = pc->io_current_outstanding->fetch_add(
            1, simt::memory_order_relaxed) + 1;
        pc->io_submitted_cnt->fetch_add(1, simt::memory_order_relaxed);
        const uint64_t bin = depth < pc->io_depth_histogram_bins
            ? depth
            : pc->io_depth_histogram_bins - 1;
        pc->io_depth_histogram[bin].fetch_add(1, simt::memory_order_relaxed);
    }
    uint32_t head, head_;
    uint64_t pc_pos;
    ...
    uint32_t cq_pos = cq_poll(&qp->cq, cid, &head, &head_);
    if (pc->io_depth_profile_enabled->load(simt::memory_order_relaxed) != 0) {
        pc->io_completed_cnt->fetch_add(1, simt::memory_order_relaxed);
        pc->io_current_outstanding->fetch_sub(1, simt::memory_order_relaxed);
    }
